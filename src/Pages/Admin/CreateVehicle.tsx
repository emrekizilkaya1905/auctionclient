...
function CreateVehicle() {
  ...
  const handleCreateOrUpdateChange = async (
    ...
    formData.append(
      "ManufacturingYear",
      vehicleModel.manufacturingYear.toString()
    );
    ...
    formData.append(
      "AdditionalInformation",
      vehicleModel.additionalInformation
    );
    ...
    formData.append("File", imageToBeStore !== undefined ? imageToBeStore : "");

    // CREATE
    if (!vehicleId) {
      formData.append("Image", imageToBeDisplay);

      const response: any = await createVehicleMutation(formData);
      if (response?.data?.isSuccess) {
        ToastrNotify("Create process is successful", "success");

        // Sayfayı yeniden yükleyerek güncel verilerin gelmesini sağla
        window.location.reload();
      } else {
        ToastrNotify("Create failed", "error");
      }
    }
    ...
    if (vehicleId && data) {
      ...
      const response: any = await updateVehicleMutation({
        createVehicle: formData,
        vehicleId,
      });
      if (response?.data?.isSuccess) {
        ToastrNotify("Update process is successful", "success");

        // Sayfayı yeniden yükleyerek güncel verilerin gelmesini sağla
        window.location.reload();
      } else {
        ToastrNotify("Update failed", "error");
      }
    }
  };
        ...
        <div className="text-center">
          <div className="mb-3">
            ...
            <input
              ...
              onChange={(e) => {
                setVehicleModelState((prevState) => {
                  return { ...prevState, brandAndModel: e.target.value };
                });
              }}
            />
          </div>
          ...
          <div className="mb-3">
            ...
            <input
              ...
              onChange={(e) => {
                setVehicleModelState((prevState) => {
                  return { ...prevState, price: parseInt(e.target.value) };
                });
              }}
            />
          </div>
          <div className="mb-3">
            ...
            <input
              ...
              onChange={(e) => {
                setVehicleModelState((prevState) => {
                  return { ...prevState, millage: parseInt(e.target.value) };
                });
              }}
            />
          </div>
          <div className="mb-3">
            ...
            <input
              ...
              onChange={(e) => {
                setVehicleModelState((prevState) => {
                  return { ...prevState, plateNumber: e.target.value };
                });
              }}
            />
          </div>
            ...
            <input
              ...
              onChange={(e) => {
                setVehicleModelState((prevState) => {
                  return {
                    ...prevState,
                    auctionPrice: parseInt(e.target.value),
                  };
                });
              }}
            />
            ...
            <input
              ...
              defaultValue={
                data?.result?.startTime
                  ? format(parseISO(data?.result?.startTime), "yyyy-MM-dd")
                  : ""
              }
          ...
          <div className="mb-3">
            ...
            <input
              ...
              defaultValue={
                data?.result?.endTime
                  ? format(parseISO(data?.result?.endTime), "yyyy-MM-dd")
                  : ""
              }
          ...
          <div className="text-center row">
            ...
            <div className="mb-3 col">
              ...
              <input
                ...
                className="form-control"
                placeholder="true"
                onClick={() => setIsActiveState(false)}
              />
            </div>
          </div>
          <div className="mb-3">
